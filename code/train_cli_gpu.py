...
if device.type == "cuda":
    ...
    print("Using CPU. For GPU training, ensure CUDA is installed and configured correctly.")
...
async def load_and_prepare_cli_data(sequence_length=14):
    ...
    print(f"First 5 y_test values: {y_test[:5]}")
...
async def train_cli_model(X_train, y_train, X_test, y_test, model, epochs, batch_size, learning_rate, training_log_path, model_version):
        ...
        for epoch in tqdm(range(epochs), desc="Training Epochs"):
            ...
            with torch.no_grad():
                for batch_X_test, batch_y_test in test_loader:
                    ...
                    all_preds.extend(test_output.squeeze().cpu().numpy())
            ...
            tqdm.write(f'Epoch {epoch+1}/{epochs}: Train Loss: {avg_train_loss:.4f} | Test Loss: {avg_test_loss:.4f} | MAE: {mae:.3f}°F | Rounding Accuracy: {rounding_accuracy:.1f}%')
            ...
            try:
                await client.execute(
                    "INSERT INTO training_runs_log (model_version, epoch, train_loss, test_loss, mae, rounding_accuracy, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (
                        model_version,
                        epoch + 1,
                        avg_train_loss,
                ...
                tqdm.write(f"Error inserting training log for epoch {epoch+1}: {e}")

            if mae < best_mae:
                best_mae = mae
                tqdm.write(f'  *** New best MAE: {best_mae:.3f}°F ***')

    pd.DataFrame(training_log).to_csv(training_log_path, index=False)
    return best_mae, training_log[-1]['rounding_accuracy']
...
def evaluate_cli_model(X_test, y_test, model):
    ...
    model.to(device)
    with torch.no_grad():
        predictions = model(torch.from_numpy(X_test).float().to(device)).squeeze().cpu().numpy()
    ...
    tqdm.write(f"Test MAE: {mae:.3f}°F")
    tqdm.write(f"Test RMSE: {rmse:.3f}°F")
    tqdm.write(f"Rounding Accuracy: {rounding_accuracy:.1f}%")
    tqdm.write(f"Within 1°F: {within_1f:.1f}%")
    
    # Show example predictions
    tqdm.write("\n=== Sample Predictions ===")
    for i in range(min(10, len(predictions))):
        tqdm.write(f"Predicted: {predictions[i]:.1f}°F | Actual: {y_test[i]:.1f}°F | Rounded: {rounded_preds[i]:.0f}°F vs {rounded_targets[i]:.0f}°F")
    
    return mae, rounding_accuracy

async def init_turso_logging_db():
    """Initializes the Turso database tables for logging and metadata."""
    async with libsql_client.create_client(url=DB_URL, auth_token=AUTH_TOKEN) as client:
        await client.execute('''
            CREATE TABLE IF NOT EXISTS training_runs_log (
...
async def main():
    ...
    print("=== MARLIN v3: CLI Settlement Training (GPU Enabled) ===")

    parser = argparse.ArgumentParser(description="Train MARLIN v3 CLI model with GPU support and versioning.")
    parser.add_argument('--model_version', type=str, help="Semantic version for the model (e.g., v1.0.0_marine)")
    parser.add_argument('--epochs', type=int, help="Number of training epochs")
    parser.add_argument('--batch_size', type=int, help="Training batch size")
    parser.add_argument('--learning_rate', type=float, help="Learning rate for optimizer")
    parser.add_argument('--notes', type=str, default="", help="Optional notes for this training run")
    args = parser.parse_args()

    # Load parameters from .env or use CLI args/defaults
    model_version = args.model_version or os.getenv("MODEL_VERSION")
    epochs = args.epochs or int(os.getenv("EPOCHS", 150))
    batch_size = args.batch_size or int(os.getenv("BATCH_SIZE", 32))
    learning_rate = args.learning_rate or float(os.getenv("LEARNING_RATE", 0.001))
    notes = args.notes

    if not model_version:
        print("Error: MODEL_VERSION not provided.")
        raise ValueError("MODEL_VERSION must be provided via CLI argument or .env file.")

    model_dir = os.path.join('model', model_version)
    if os.path.exists(model_dir):
        print(f"Error: Model version '{model_version}' already exists.")
        raise FileExistsError(f"Model version '{model_version}' already exists at {model_dir}. Please choose a new version or delete the existing one.")
    ...
    # Initialize Turso logging database
    print("Initializing Turso logging database...")
    await init_turso_logging_db()
    print("Turso logging database initialized.")

    # Load CLI-targeted data
    print("Loading and preparing CLI data...")
    (X_train, X_test, y_train, y_test), scaler = await load_and_prepare_cli_data()
    print("CLI data loaded and prepared.")
    ...
    # Train the model
    print(f"\n=== Training CLI Model ===")
    final_mae, final_accuracy = await train_cli_model(X_train, y_train, X_test, y_test, model, epochs, batch_size, learning_rate, training_log_path, model_version)
    ...
    # Save metadata to Turso DB
    try:
        async with libsql_client.create_client(url=DB_URL, auth_token=AUTH_TOKEN) as client:
            await client.execute(
                "INSERT OR REPLACE INTO model_metadata (model_version, training_date, input_feature_names, final_mae, final_accuracy, training_parameters, notes) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    metadata["model_version"],
                    metadata["training_date"],
                    json.dumps(metadata["input_feature_names"]),
                    metadata["final_mae"],
                    metadata["final_accuracy"],
    ...
    print(f"✅ CLI model saved to '{model_path}'")
    print(f"✅ CLI scaler saved to '{scaler_path}'")
    print(f"✅ Training log saved to '{training_log_path}'")
    print(f"✅ Metadata saved to '{metadata_path}'")
    print(f"✅ Training logs and metadata saved to Turso DB.")
